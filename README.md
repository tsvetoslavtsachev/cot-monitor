...
### Етикети на лицето (КОКПИТ O3/O4, вълна 1)

- **O4 бадж** под заглавието: `Наблюдение, не сигнал · coincident` — уредът е дескриптивен позиционен инвентар, не trade-сигнал и не изпреварва пазара.
- **Прозорец на перцентила** (O3 §1 rule 2/3): всеки перцентил носи прозореца си на лицето — `пълна история, N седм — несравним между пазари`. N варира per пазар (229→1046 седм), затова 90-и в един пазар ≠ 90-и в друг. Полето `percentile_window` е и в `watchlist.json` (изгражда се от `derive_metrics.percentile_window_label`).
- **Клас на доверие** (M5, одит 07.07): винаги-видима легенда + чип на всеки ред подреждат класовете по доверие низходящо — Лихви (единственият реален контрариан, IC −0.25) → FX → метали → ⚠ енергия/агро (континюейшън-капан) → индекси (шум, IC≈0). Аргумент за анализа, не решение. Пълната бележка е в `class_trust_note` (tooltip + разгъната разбивка).
